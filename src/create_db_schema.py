...
conn.cursor().execute("CREATE TABLE Organizations (id SERIAL PRIMARY KEY, name TEXT, code TEXT);")
conn.cursor().execute("CREATE TABLE OrgMembers (member INTEGER REFERENCES Users, org INTEGER REFERENCES Organizations, admin BOOLEAN);")
conn.commit()